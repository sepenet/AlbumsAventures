...
import type { AlbumDetail, Category } from "../types/api";
...
/**
 * SPA-native album edit (`/app/album/:id/edit`, superuser-only). Replaces the
 * retired Jinja `album_edit.html`. Flow: prefill from `GET
 * get_album_by_id/{id}` (DB->Web list transform) -> `PATCH update_album/{id}`
 * -> optional `POST upload_cover/{id}` -> navigate to the album detail page. A
 * cover-only failure is surfaced without discarding the saved metadata edit.
 *
 * Parity with the Jinja form: a warning banner reminds the editor that changing
 * the title/date/participants triggers a server-side album-directory rename
 * (`update_album` -> `rename_album_folder`), which moves existing media files.
 */
export function AlbumEditPage() {
  ...
  // Category modal state (parity with the create page's inline-create affordance).
  const [modalOpen, setModalOpen] = useState(false);
  const [newCategoryName, setNewCategoryName] = useState("");
  const [categoryError, setCategoryError] = useState<string | null>(null);

  const albumQuery = useQuery<AlbumDetail>({
    queryKey: ["album", albumId],
    queryFn: () => api.get<AlbumDetail>(`/be_album/get_album_by_id/${albumId}`),
  ...
  // Prefill the controlled form once the album read resolves (DB->Web transform).
  useEffect(() => {
    if (albumQuery.data && !prefilled) {
      setValues(toFormValues(albumQuery.data));
      setPrefilled(true);
    }
  }, [albumQuery.data, prefilled]);

  const currentCover = albumQuery.data?.image_cover ?? null;
  ...
  const submit = useMutation({
    mutationFn: () =>
      runUpdateAlbum(albumId, toWritePayload(values), cover, {
        updateAlbum: (id, payload) => api.patch(`/be_album/update_album/${id}`, payload),
        uploadCover: (id, file) => {
          const fd = new FormData();
          fd.append("image_cover", file);
          return api.postForm(`/be_album/upload_cover/${id}`, fd);
        },
      }),
    onSuccess: (outcome) => {
      ...
      if (outcome.status === "saved") {
        ...
        // Metadata edit succeeded but the new cover failed — keep the user on the
        // form so they can retry the upload without losing the saved changes.
        setSubmitError(
          "Les modifications ont été enregistrées, mais l'envoi de la nouvelle couverture a échoué. Vous pouvez réessayer.",
        );
      }
    },
    onError: (error) => setSubmitError(errorText(error, "Erreur lors de la modification de l'album")),
  });
  ...
  if (albumQuery.isPending && Number.isFinite(albumId)) {
    return <p className="py-12 text-center text-gray-500 dark:text-gray-400">Chargement de l'album…</p>;
  }
    ...
    return (
      <div className="py-12 text-center">
        <p className="text-red-600 dark:text-red-400">
          {notFound ? "Album introuvable." : errorText(albumQuery.error, "Erreur lors du chargement de l'album.")}
        </p>
        <button type="button" onClick={() => navigate("/")} className={`${secondaryBtn} mt-4`}>
          Retour aux albums
        </button>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-2xl py-4">
      <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-gray-100">Modifier l'album</h1>

      {/* Parity warning: metadata edits rename the album directory server-side. */}
      <div
        role="note"
        className="mb-6 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300"
      >
        Modifier le titre, la date ou les participants renomme le dossier de l'album sur le serveur
        et y déplace les fichiers médias existants.
      </div>
      ...
      <form onSubmit={onSubmit} className={`${cardClass} space-y-5 p-6`} noValidate>
        ...
        <div>
          ...
          {currentCover && !coverPreview ? (
            <p className="mb-2 text-sm text-gray-500 dark:text-gray-400">
              Couverture actuelle : <span className="font-medium">{currentCover}</span>
            </p>
          ) : null}
          <div
            onDragOver={(event) => {
          ...
          >
            {coverPreview ? (
              <img src={coverPreview} alt="Aperçu de la nouvelle couverture" className="max-h-48 rounded-lg object-contain" />
            ) : (
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Glissez-déposez une image ou cliquez pour remplacer la couverture (optionnel).
              </p>
            )}
            <input
              ref={fileInputRef}
        ...
        <div className="flex justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-700">
          <button type="button" onClick={() => navigate(`/albums/${albumId}`)} className={secondaryBtn}>
            Annuler
          </button>
